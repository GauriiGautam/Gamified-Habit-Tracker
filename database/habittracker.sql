...
CREATE TABLE USER (
    ...
    FOREIGN KEY (CurrentLevel) REFERENCES LEVEL(LevelID)
);
...
CREATE TABLE REWARD (
    ...
    UnlockLevel INT,
    IsActive    INT DEFAULT 1,
    FOREIGN KEY (UnlockLevel) REFERENCES LEVEL(LevelID)
);
...
CREATE TABLE HABIT_SHARING (
    ...
    FOREIGN KEY (HabitID)        REFERENCES HABIT(HabitID),
    FOREIGN KEY (SharedByUserID) REFERENCES USER(UserID),
    FOREIGN KEY (SharedToUserID) REFERENCES USER(UserID)
);
